...
export const ControlSlider: React.FC<ControlSliderProps> = ({
  ...
  return (
    <div
      className={`flex flex-col space-y-2 transition-all duration-200 ${disabled ? 'pointer-events-none opacity-50' : 'hover:scale-[1.01]'}`}
    >
      <div className="flex items-center justify-between">
        <Label>{label}</Label>
        <span
          className={`flex items-center rounded-sm px-2 py-0.5 text-xs transition-all duration-300 ${
            isAnimating
