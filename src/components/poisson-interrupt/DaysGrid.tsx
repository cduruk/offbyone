        ...
        {day.blocks.map((block, bIdx) => {
          ...
          return (
            <rect
              ...
              x={blockX}
              y={hasGoodBlock ? 3 : 0}
              width={Math.max(blockW - Math.min(blockW * 0.1, 0.5), 0.1)}
              height={hasGoodBlock ? DAY_HEIGHT - 6 : DAY_HEIGHT}
...
export const DaysGrid: React.FC<DaysGridProps> = ({
  ...
  return (
    <div
      ref={gridRef}
      className="days-grid-container relative w-full max-w-[800px]"
    >
      {/* Grid Legend */}
      <div className="mb-4 flex flex-wrap items-center justify-center gap-4">
        <span className="flex items-center gap-1.5">
          <span className="h-2.5 w-2.5 rounded-sm bg-teal-500"></span>
          <LabelSmall>45+ min</LabelSmall>
        </span>
        <span className="flex items-center gap-1.5">
          <span className="h-2.5 w-2.5 rounded-sm bg-indigo-500"></span>
      ...
      <svg
        ...
        className="animate-in fade-in zoom-in-95 h-auto w-full drop-shadow-sm duration-300"
      ...
      {hoveredDay && (
        <div
          className="pointer-events-none absolute z-50 mt-[-10px] -translate-x-1/2 -translate-y-full transform rounded-sm bg-gray-900 p-3 text-white shadow-xl"
